...
axs[0].set_xlabel('Actual Quantity')

sns.histplot(data=df, x='defect_quantity', kde=False, ax=axs[1])
axs[1].set_title('Histogram of Defect Quantity')
axs[1].set_xlabel('Defect Quantity')
plt.tight_layout()
plt.savefig('output/production_orders_histogram.png')
plt.show()

# Visualization 2: Boxplot of actual_quantity by order_status and shift
fig, ax = plt.subplots(figsize=(10, 6))
sns.boxplot(x='order_status', y='actual_quantity', hue='shift', data=df, ax=ax)
ax.set_title('Boxplot of Actual Quantity by Order Status and Shift')
ax.set_xlabel('Order Status')
ax.set_ylabel('Actual Quantity')
plt.tight_layout()
